...
class ResourceFieldTest extends IntegrationTest
{
    ...
    public function test_uses_index_fields()
    {
        ...
        $this->assertSame(
            ['ID', 'Name (Email)', 'Index Name'],
            $resource->indexFields($request)->pluck('name')->all()
        );

        $this->assertCount(4, $resource->availableFields($request));
        $this->assertCount(3, $resource->indexFields($request));
    }
    ...
    public function test_uses_detail_fields()
    {
        ...
        $this->assertSame(
            ['ID', 'Weight', 'Detail Name', 'Restricted', 'Avatar'],
            $resource->detailFields($request)->pluck('name')->all()
        );

        $this->assertCount(6, $resource->availableFields($request));
        $this->assertCount(5, $resource->detailFields($request));
    }
    ...
    public function test_uses_create_fields()
    {
        ...
        $this->assertCount(5, $resource->availableFields($request));
